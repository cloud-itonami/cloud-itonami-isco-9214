(ns gardenhort.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [gardenhort.store :as store]
            [gardenhort.advisor :as advisor]
            [gardenhort.governor :as governor]))

(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-worker! st {:worker-id "worker-1" :name "Aiko Tanaka"})
    (store/register-site! st {:site-id "SITE-1" :name "Tanaka Botanical Gardens" :max-supply-cost 2000})
    st))
...
(deftest hard-on-op-outside-closed-allowlist
  (let [st (fresh-store)
        v (governor/check req {} (op :dispatch-mower) st)]
    (is (:hard? v))
    (is (some #(= :unknown-op (:rule %)) (:violations v)))))

(deftest hard-on-scope-excluded-op-finalize-garden-maintenance-operation
  (testing "finalizing a garden-work-execution decision (the garden-maintenance operation) is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-garden-maintenance-operation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-approve-garden-maintenance-procedure
  (testing "approving a garden maintenance procedure is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :approve-garden-maintenance-procedure) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-finalize-garden-maintenance-operation
  (testing "defense-in-depth: a rationale that itself attempts to finalize the garden-maintenance operation is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we finalize the garden-maintenance operation now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a power-tool-hazard/pesticide-exposure/weather-exposure concern always requires human sign-off"
    (let [st (fresh-store)
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'mower'/'trimmer'/'pesticide'/'site safety supervisor', but never the full finalization-action phrases)"
    (let [st (fresh-store)
      ...
      (doseq [o ops]
        (let [request {:worker-id "worker-1" :op o :site-id "SITE-1"
                        :stake :low :task "routine mower and trimmer maintenance task"
                        :hazard-type :pesticide-exposure :cost 500}
